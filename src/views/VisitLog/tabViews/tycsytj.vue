  ...
  <div>
    ...
    >
      ...
      <el-table-column prop="segmentModule" label="细分模块" align="center">
        <template slot-scope="scope">{{ scope.row.segmentModule || '/' }}</template>
      </el-table-column>
      <el-table-column prop="num" label="总次数" align="center"> </el-table-column>
      ...
      <el-table-column prop="surplusNum" label="剩余次数" align="center">
        <template slot-scope="scope">{{ scope.row.surplusNum || '/' }}</template>
      </el-table-column>
    </el-table>
    <div class="title" style="margin-top:20px">客商初筛调用次数</div>
    <el-table :data="applyData" border style="width: 60%;" size="small" :header-cell-style="{ background: '#ECF1FE' }">
      <el-table-column prop="COMPANYNAME" label="申请企业" align="center"> </el-table-column>
